...
data class Path (val type: PathType, val target: Target2D, val velocity: Double){
    ...
    constructor(heading:Angle,velocity: Double, currentTarget: Target2D): this(PathType.TURN_TO, Target2D(currentTarget.x,currentTarget.y,heading), velocity)
    constructor(heading:Angle,velocity: Double, x: Double, y: Double): this(PathType.TURN_TO, Target2D(x,y,heading), velocity)
